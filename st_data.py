...
class BaselineDataset(torch.utils.data.Dataset):
    ...
    def get_img(self, name: str):
        ...
        if self.data == 'her2st':
            ...
            path = pre+'/'+fig_name
        elif self.data == 'stnet' or ('10x_breast' in self.data) or ('NCBI' in self.data):
...
class STDataset(BaselineDataset):
    ...
    def __init__(self, 
        ...
        names.sort()
        names = [i.split('_selection.tsv')[0] for i in names] #A1/A2
    ...
    def __getitem__(self, index):
            ...
            for j in range(n_patches):
                ...
                if self.extract_mode == 'neighbor':
                        ...
                        for k, (k_start, k_end) in enumerate(k_ranges):
                            for m, (m_start, m_end) in enumerate(m_ranges):
                                ...
                                if mask_tb[j, n] != 0:
                                    ...
                                    target_shape = (2*self.r, 2*self.r, 3)
                                    pad_height = target_shape[0] - tmp.shape[0]
                                    pad_width = target_shape[1] - tmp.shape[1]
                                    pad_depth = target_shape[2] - tmp.shape[2]
                                    pad_values = (
                                        (0, pad_height if pad_height > 0 else 0),
                                        (0, pad_width if pad_width > 0 else 0),
                                        (0, pad_depth if pad_depth > 0 else 0)
                                    )
                                    tmp = np.pad(tmp, pad_values, mode='constant', constant_values=0)
                                    tmp = tmp[:2*self.r, :2*self.r, :3]
                    ...
                    if self.use_pyvips:
                        ...
                        target_shape = (2*self.r, 2*self.r, 3)
                        pad_height = target_shape[0] - patch.shape[0]
                        pad_width = target_shape[1] - patch.shape[1]
                        pad_depth = target_shape[2] - patch.shape[2]
                        pad_values = (
                            (0, pad_height if pad_height > 0 else 0),
                            (0, pad_width if pad_width > 0 else 0),
                            (0, pad_depth if pad_depth > 0 else 0)
                        )
                        patch = np.pad(patch, pad_values, mode='constant', constant_values=0)
                        patch = patch[:2*self.r, :2*self.r, :3]
        ...
        if self.mode == 'train':
            ...
            return patches, exps, sid, wsi, position, name, neighbors, mask_tb, centers
