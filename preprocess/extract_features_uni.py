...
def get_sub_features(model, patches, num_n):
    ...
    for m in range(num_n):
        for k in range(num_n):
            ...
            features = model(patch.to(f'cuda:{args.gpu}'))
...
if __name__=='__main__':
    ...
    parser.add_argument('--config', type=str, default='skinst/dml', help='logger path.')
    parser.add_argument('--test_name', type=str, default='NCBI463', help='dataset name:{"10x_breast_ff1","10x_breast_ff2", "10x_breast_ff3", "DRP1", "DRP2", "NCBI463", "NCBI464"}.')
    ...
    #   
    model.load_state_dict(torch.load("./weights/pytorch_model.bin"))
    model = model.to(f'cuda:{args.gpu}')  # Move model to GPU
    model.eval()
            
    # Name of directory to store the extracted features
    if extract_mode == 'target':
        save_dir = f"gt_features_uni_small"
    elif extract_mode == 'neighbor':
        save_dir = f"n_features_{num_n}_uni_small"
    ...
    if mode == 'internal':
        ...
        for fold in tqdm(range(num_k)):
            ...
            for i, patches in enumerate(dataloader):
                ...
                if extract_mode == 'target':
                    patches = patches.to(torch.device(f'cuda:{args.gpu}')).squeeze()
                else:
        ...
        dir_name=f"{data_dir}/{test_name}/{save_dir}_224"
        os.makedirs(dir_name, exist_ok=True)
        ...
        for i, patches in tqdm(enumerate(dataloader)):
            ...
            for patch in patches:
                if extract_mode == 'target':
                    patch = patch.to(f'cuda:{args.gpu}').squeeze()
                else:
                    patch = patch.squeeze()
